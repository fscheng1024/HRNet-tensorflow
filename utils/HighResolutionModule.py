...
class HighResolutionModule():
    ...
    def _make_one_branch(self, x, branch_index, block, num_blocks, num_channels,
        ...
        x = block(
            ...
            block_name=self.stage + '_Botteleneck0' + str(branch_index), 
            outplanes=num_channels[branch_index] * 1, 
            stride=stride, 
        ...
        for i in range(1, num_blocks[branch_index]):
            x = block(
                x, 
                is_training=self._is_training, 
                block_name=self.stage + 'Botteleneck' + str(i) + str(branch_index), 
                outplanes=num_channels[branch_index]
            )
            ...
            for j in range(num_branches):
                if j > i:
                    with tf.variable_scope(self.stage + '_fuse_layers' + str(i)+str(j)):
                        tmp = block_Model.conv(x[j], 1, num_inchannels[i], 'conv_', stride=1)
                        tmp = block_Model.batch_norm(tmp, self._is_training)
                        tmp = tf.image.resize_bilinear(tmp, x[i].get_shape().as_list()[1:3])
                        tmp_layer.append(tmp)
                elif j == i:
                    tmp_layer.append(x[j])
                else:
                    with tf.variable_scope(self.stage + '_fuse_layers' + str(i) + str(j)):
                        tmp = block_Model.conv(x[j], 3, num_inchannels[i], 'conv_', stride=2)
                        tmp = block_Model.batch_norm(tmp, self._is_training)
                        tmp_layer.append(tmp)
            for k in range(len(tmp_layer)):
                fuse = 0
                fuse = fuse + tmp_layer[k]
