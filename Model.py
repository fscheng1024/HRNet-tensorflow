...
from utils import HighResolutionModule as HRModule
...
class HighResolutionNet():
    ...
    def forword(self, x):
        ...
        with tf.variable_scope('stage1'):
            with tf.variable_scope('stage1_conv1'):
                tmp = block_Model.conv(x, 3, 64, 'stage1_conv1', stride=2)
                tmp = block_Model.batch_norm(tmp, self._is_training)
            with tf.variable_scope('stage1_conv2'):
                tmp = block_Model.conv(tmp, 3, 64, 'stage1_conv2', stride=2)
                tmp = block_Model.batch_norm(tmp, self._is_training)
            layer1 = self._make_layer(tmp, block_Model.Bottleneck, 64, 64, 4)
        ...
        with tf.variable_scope('stage2'):
            ...
            self.layer = self._make_transition_layer(
                'stage2', self.layer,[256], num_channels
            )
            self.layer, pre_stage_channels = self._make_stage(
                'stage2', self.layer,config.HIGH_RESOLUTION_NET[1], num_channels
            )
        ...
        with tf.variable_scope('stage3'):
            ...
            num_channels = [num_channels[i] * 1 for i in range(len(num_channels))]
            self.layer = self._make_transition_layer(
                'stage3', self.layer,pre_stage_channels, num_channels
            )
            self.layer, pre_stage_channels = self._make_stage(
                'stage3', self.layer, config.HIGH_RESOLUTION_NET[2], num_channels
            )
        ...
        with tf.variable_scope('stage4'):
            ...
            num_channels = [num_channels[i] * 1 for i in range(len(num_channels))]
            self.layer = self._make_transition_layer(
                'stage4', self.layer, pre_stage_channels, num_channels
            )
            self.layer, pre_stage_channels = self._make_stage(
                'stage4', self.layer, config.HIGH_RESOLUTION_NET[3], num_channels
            )
        ...
        with tf.variable_scope('concat'):
            ...
            feature_map = block_Model.conv(feature_map, 1, self.num_class, 'concat_conv', 1)
    ...
    def _make_layer(self, x, block, inplanes, planes, blocks, stride=1):
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            ...
            downsample.add(block_Model.conv(x, 1, planes*block.expansion, 'concat_conv', stride=1))
            downsample.add(block_Model.batch_norm(planes*block.expansion, self._is_training))
            ...
            if i < num_branches_pre:
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    with tf.variable_scope(stage + 'stage1_transition_layers_2'+str(j)):
                        tmp = block_Model.conv(layer[i-1], 3, outchannels, 'transition_layers', 2)
                        tmp = block_Model.batch_norm(tmp, self._is_training)
                        conv3x3s.append(tmp)
    ...
    def _make_stage(self, stage, layer,layer_config, num_inchannels,
        ...
        for i in range(num_modules):
            ...
            hr_module = HRModule.HighResolutionModule(
                                        stage, 
                                        self._is_training, 
                                        num_branches, 
                                        block, 
            ...
            make_stage, channel = hr_module.forword(
                                        layer, 
                                        num_branches, 
                                        block,
                                        num_blocks, 
